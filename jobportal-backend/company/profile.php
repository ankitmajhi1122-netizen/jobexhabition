...
if ($user['role'] !== 'company') {
    sendError('Access denied. Companies only.', 403);
}
...
if ($method === 'GET') {
    ...
    sendSuccess('Company Profile', $profile);
    ...
    if ($exists) {
        ...
        $sql = "INSERT INTO companies (user_id, company_name, registration_no, contact_person, phone, address, website, is_consultancy) VALUES (?, ?, ?, ?, ?, ?, ?, 0)";
        $stmt = $pdo->prepare($sql);
